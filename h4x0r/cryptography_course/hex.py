from termcolor import colored
c1 = ""
c2 = '234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f'
c3 = '32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb'
c4 = ''
c5 = '3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070'
c6 = ''
c7 = ''
c8 = '315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3'
c9 = ''
c10 = '466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83'
c11 = '32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904'

cyphert = [c1, c2, c3, c4, c5, c6, c7, c8, c9, c10, c11]
split_cyphert = []
compared_cyphert = []
words_to_compare = []

for cyphertext in cyphert:
    split_cyphert.append( [ op + code for (op, code) in zip(cyphertext[0::2], cyphertext[1::2])] )

for x in range(0, 10):
    compared_cyphert.append( [ int(t1, 16)^int(t2, 16) for t1, t2 in zip(split_cyphert[10], split_cyphert[x]) ] )

words_to_compare = []
words_to_compare.append([ord(x) for x in list('We can factor the number 15 with quantum computers. We can also factor ')])
words_to_compare.append([ord(x) for x in list('Euler would probably enjoy that now his theorem becomes a')])
words_to_compare.append([ord(x) for x in list('The nice thing about Keeyloq is now we cryptographers can ')])
words_to_compare.append([ord(x) for x in list('The cyphertext produced by a weak encryption algorithm lo')])
words_to_compare.append([ord(x) for x in list("You don't want to buy a set of car keys from a guy who sp")])
words_to_compare.append([ord(x) for x in list('There are two types of cryptography - that which will keep ')])
words_to_compare.append([ord(x) for x in list('There are two types of cryptogrpahy: one that allows the G')])
words_to_compare.append([ord(x) for x in list('We can see the point where the chip is unhappy id a wrong ')])
words_to_compare.append([ord(x) for x in list('A (private-key)  encryption scheme states 3 algorithms, number one')])
secret_message = [ord(x) for x in list('The secret message is: When using a stream cipher, never use the key more than once')]
print('The secret message is: When using a stream cipher, never use the key more than once')

def iterate_chunk(cur_list, size):
    count = 0
    while count <= (len(cur_list) - size):
        yield cur_list[count:count+size]
        count += 1

for current_text, current_word in zip(compared_cyphert, words_to_compare):
    count = 0
    for some_chunck in iterate_chunk(current_text, len(secret_message)):
        count += 1
        print(count, colored('|', 'red'), ''.join([chr(x^y) for x,y in zip(some_chunck, secret_message)]), colored('|', 'red'), end='   ')
        print()
    count = 0
    print(colored(''.join([chr(x) for x in current_word]), 'green'))
    for current_chunk in iterate_chunk(current_text, len(current_word)):
        count += 1
        print(count, colored('|', 'red'), ''.join([chr(x^y) for x,y in zip(current_word, current_chunk)]), colored('|', 'red'), end='   ')
        print()
    input()

"""
c1:
    message is:
        11. Tor the num
c2:
    message is:
        11. d probably
c3:
    message is:
        11. hing about Q
c4:
    message is:
        11. text produc 
    text produc:
        10. & message is
c5:
    message is:
        want to buy:
c6:
    message is:
        two types o|
c7:
    message is:
        two types o|
c8:
    message is:
        the point m
c9:
    message is:
        -key) encrc
c10:
    message is:
        
"""
