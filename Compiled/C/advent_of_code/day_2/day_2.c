...
int main() {
    ...
    int count_1 = 0;
    int count_2 = 0;
    char *data = line_of_ints;

    // Read data until end of line
    // Input is normally 16 lines of 16 ints but I made it one line and now i'm splitting it into a 2d array
    while (sscanf(data, " %d%n", &n, &offset) == 1) {
        if (count_2 == 16) { count_2 = 0; count_1++; }
        test[count_1][count_2] = n;
        data += offset;
        count_2++;
    }
    ...
    for (int x=0;x<16;x++) {
        bubble_sort(test[x]);
        e_sum += easy_line_checksum(test[x]);
        h_sum += hard_line_checksum(test[x]);
        for (int y=0;y<16;y++) {
            printf("%d ", test[x][y]);
        }
        printf(" -----> %d\n", hard_line_checksum(test[x]));
    }
    printf("sum: %d", h_sum);
    return 0;
}
...
int easy_line_checksum(int line[16]) {
    return line[15] - line[0];
}

//Brute force solution but it works so whatevs
int hard_line_checksum(int line[16]) {
    for (int end=15; end > 0; end--) {
        for (int i=0; i < end; i++) {
            if (line[end] % line[i] == 0) { return line[end] / line[i]; }
        }
    }
}
